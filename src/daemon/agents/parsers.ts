...
//
// OpenAI-compatible streaming: the response body is `text/event-stream` with
// `data: {...}\n\n` framed events. Each `data:` payload is a JSON
// chat-completion-chunk. With `stream_options: {include_usage: true}` the
// terminal chunk carries `usage.{prompt_tokens, completion_tokens, cost}` —
// `cost` is OpenRouter's own field (USD, not in upstream OpenAI). The stream
// closes with a literal `data: [DONE]` sentinel.
//
// We accept ONE pre-stripped line at a time (caller has already split on
// `\n\n` and stripped the `data: ` prefix). Empty / `[DONE]` / non-JSON
// lines return [] so the caller can pass everything through uniformly.
//
...
//
// We emit `text_delta` for each delta.content, and a single `message_done`
// when we see `finish_reason !== null`. The usage chunk (separate from the
// finish chunk) augments the most recent message_done's `usage` field via a
// stateful pair of helpers (parseOpenRouterSSE handles a single line; the
// shim's runHeadless aggregates usage onto the message_done).
export function parseOpenRouterSSE(line: string): AgentEvent[] {
  const trimmed = line.trim();
  if (trimmed.length === 0) return [];
  if (trimmed === '[DONE]') return [];
  // OpenRouter passes through OpenAI-style "OPENROUTER PROCESSING" comments
  // on slow upstreams as `: <message>` — these aren't JSON. Drop silently.
  if (trimmed.startsWith(':')) return [];

  const obj = tryJson(trimmed) as Record<string, unknown> | undefined;
  if (!obj) return [];

  // Error envelope (e.g. invalid model, rate limit). OpenRouter wraps these
  // as `{"error":{"message":"...","code":...,"metadata":...}}`.
  ...
  if (Array.isArray(choices) && choices.length > 0) {
    for (const c of choices) {
      ...
      // finish_reason is set ('stop' | 'length' | 'content_filter' | ...) on
      // the terminal chunk. We don't emit message_done here directly because
      // the usage-bearing chunk often arrives AFTER the finish chunk; the
      // shim assembles the final event with usage attached.
    }
  }

  // Usage chunk — OpenRouter sends this LAST when stream_options.include_usage
  // is set. cost is OpenRouter-specific (USD). Emit a synthetic message_done
  // with empty finalText so the shim/runner can attach usage; the runner's
  // accumulator already holds the assembled finalText.
